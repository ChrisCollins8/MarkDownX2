...
namespace ScintillaNET
{
    ...
    public class FindReplace : TopLevelHelper
    {
        ...
        public unsafe Range Find(int startPos, int endPos, string searchString, SearchFlags flags)
        {
            ...
            byte[] bytes = Scintilla.Encoding.GetBytes(searchString);
            fixed (byte* pb = bytes)
            {
                ...
                if (pos >= 0)
                {
                    return new Range(pos, pos + bytes.Length, Scintilla);
                }
                else
                {
                    return null;
        ...
        public Range Find(Range r, Regex findExpression, bool searchUp)
        {
            ...
            if (searchUp)
            {
                ...
                while (m.Success)
                {
                    int start = r.Start + Scintilla.Encoding.GetByteCount(text.Substring(0, m.Index));
                    int end = Scintilla.Encoding.GetByteCount(text.Substring(m.Index, m.Length));

                    range = new Range(start, start + end, Scintilla);
                    m = m.NextMatch();
                }
            ...
            else
            {
                ...
                return new Range(start, start + end, Scintilla);
            }
        }
...
//                return Find(searchRange.Start, searchRange.End, searchString, flags);
            ...
            Range r = Find(searchRange.Start, searchRange.End, searchString, flags); ; // Find(caret, searchRange.End, searchString, flags);
            if (r != null)
                return r;
            else if (wrap)
        ...
        public void HighlightAll(IList<Range> foundRanges)
        {
            ...
            {
                r.SetIndicator(Indicator.Index);
            }
        }
        ...
        public void ShowFind()
        {
            ...
            Window.tabAll.SelectedTab = Window.tabAll.TabPages["tpgFind"];

            Range selRange = Scintilla.Selection.Range;
            if (selRange.IsMultiLine)
            {
                Window.chkSearchSelectionF.Checked = true;
            }
            else if (selRange.Length > 0)
            {
                Window.cboFindF.Text = selRange.Text;
            }

            Window.cboFindF.Select();
            Window.cboFindF.SelectAll();
        ...
        protected internal FindReplace(Scintilla scintilla) : base(scintilla)
        {
