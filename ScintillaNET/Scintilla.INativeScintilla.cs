...
namespace ScintillaNET
{
    public partial class Scintilla
    {
        ...
        private void FireModified(NativeScintillaEventArgs ea)
        {
            ...
            if((modType & TEXT_MODIFIED_FLAGS) > 0)
            {
                TextModifiedEventArgs mea = new TextModifiedEventArgs
                    ...
                    scn.length,
                    scn.linesAdded,
                    Utilities.IntPtrToString(Encoding, scn.text, scn.length)
                    );
        ...
        void INativeScintilla.StyleSetFont(int styleNumber, string font)
        {
            _ns.SendMessageDirect(Constants.SCI_STYLESETFONT, styleNumber, font);
        }
        ...
        #region Multiple selection stuff

        public void SetMultipleSelection(bool value)
        {
            _ns.SendMessageDirect(Constants.SCI_SETMULTIPLESELECTION, value);
        }
        public void SetAdditionalSelectionTyping(bool value)
        {
            _ns.SendMessageDirect(Constants.SCI_SETADDITIONALSELECTIONTYPING, value);
        }

        public void SetSelection(int caret, int anchor)
        {
            _ns.SendMessageDirect(Constants.SCI_ADDSELECTION, caret, anchor);
        }
        ...
        {
            return _ns.SendMessageDirect(Constants.SCI_GETSELECTIONS);
        }
        public void ClearSelections()
        {
            _ns.SendMessageDirect(Constants.SCI_CLEARSELECTIONS);
        }
