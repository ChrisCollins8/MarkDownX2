...
namespace ScintillaNET
{
    ...
    public class Style : ScintillaHelperBase
    {
        ...
        {
            // In the past we used the property bag to store font names to preserve their
            // original representation round-trip with Scintilla. This is no longer necessary
            // because Scintilla will give us back the exact same name we gave it.

            get
            {
                int length = (int)Scintilla.DirectMessage(NativeMethods.SCI_STYLEGETFONT, (IntPtr)_index, IntPtr.Zero);
                byte[] buffer = new byte[length];
                unsafe
                {
                    fixed (byte* bp = buffer)
                        Scintilla.DirectMessage(NativeMethods.SCI_STYLEGETFONT, (IntPtr)_index, (IntPtr)bp);
                }
                ...
                string name = Encoding.Default.GetString(buffer, 0, length);
                return name;
            }
            set
            {
                if (value != FontName)
                {
                    // TODO Check 'value' argument for null and/or valid
                    // As best I can tell, Scintilla is using the LOGFONTA structure for loading
                    // and saving fonts. Meaning we need to convert our font name to ANSI.
                    byte[] buffer = Utilities.GetZeroTerminatedBytes(value, Encoding.Default);
                    unsafe
                    {
                        fixed (byte* bp = buffer)
                            Scintilla.DirectMessage(NativeMethods.SCI_STYLESETFONT, (IntPtr)_index, (IntPtr)bp);
                    }
