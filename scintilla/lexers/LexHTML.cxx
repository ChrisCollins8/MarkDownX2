...
/*static void GetPrevWord(unsigned int pos,
                        char *prevWord,
                        Accessor &styler)
{
	unsigned int i;
	...
	for (i = 0; i < 199; i++){
		if ((pos - i) > 0){
			char ch = static_cast<char>(styler.SafeGetCharAt(pos - (i)));
			if ((i == 0) && !IsAWordStart(ch))
				break;
			if ((i > 0) && !IsAWordChar(ch))
				break;
	...
	for(i = final; i <= pos; i++){
		char ch = static_cast<char>(styler.SafeGetCharAt(i));
		*dst++ = ch;
		x++;
	}
	*dst = 0;
...
static void classifyWordHTPHP(unsigned int start, unsigned int end, WordList &keywords, Accessor &styler) {
	...
	if (wordIsNumber) {
		...
		if (keywords.InList(s)){
			...
			char prevWord[200];
			prevWord[0] = 0;
			// First check if it has a -> or a :: before and a ( after. If so then it's a 
			// method.
			if (start > 1){
				char chNext = static_cast<unsigned char>(styler.SafeGetCharAt(end + 1));
				char chPrev = static_cast<unsigned char>(styler.SafeGetCharAt(start - 1));
				char chPrev2 = static_cast<unsigned char>(styler.SafeGetCharAt(start - 2));
				if ((chPrev == '>' && chPrev2 == '-' && chNext == '(') || (chPrev == ':' && chPrev2 == ':' && chNext == '(')){
					chAttr = SCE_HPHP_FUNCTION;
				} else if (chPrev == '>' && chPrev2 == '-' && chNext != '('){
					chAttr = SCE_HPHP_PROPERTY;
				} else {
					GetPrevWord(start-1, prevWord, styler);

					// Determine if the previous word is class or extends. If it is then set the style to the
					// HTML attribute style. This way classes are highlighted.
					if (strcmp(prevWord, "class")==0 || strcmp(prevWord, "extends") == 0 || strcmp(prevWord, "new")==0){

						if (strcmp(prevWord, "new")==0){
							// Find out if the word contains the \ in it. If it does it is a 
							// namespace up until the last one. This makes it tricky because
							// we need to highlight twice in this case. Namespace highlight
							// up through the last / and then object.
							//int tempEnd = end;
							bool hasNamespace = false;
							for(int i = start; i < end; i++){
								char tChar = static_cast<unsigned char>(styler.SafeGetCharAt(i));
								if (tChar == '\\'){
									hasNamespace = true;
									break;
								}
							...
							/*if (tempEnd != end){
								styler.ColourTo(tempEnd, SCE_HPHP_NAMESPACE);
								styler.StartSegment(tempEnd);
								start = tempEnd;								
							}*/
							if (!hasNamespace){
								chAttr = SCE_HPHP_OBJECT;	
							} else {
								chAttr = SCE_HPHP_NAMESPACE;
							}
						} else {
							chAttr = SCE_HPHP_OBJECT;	
						}
						
					} else if (strcmp(prevWord, "function")==0){ // Function name so lets use a
						chAttr = SCE_HPHP_FUNCTION;
					} else if (strcmp(prevWord, "namespace")==0 || strcmp(prevWord, "use")==0){ // namespace name
						chAttr = SCE_HPHP_NAMESPACE;
					}
				}
			}
