			...
			if (ll) {
				...
				if ((level & SC_FOLDLEVELHEADERFLAG) &&
					...
					if ((expanded && (foldFlags & SC_FOLDFLAG_LINEBEFORE_EXPANDED))
						...
						// draw yellow box test
						rcFoldLine.top -= 10;
						rcFoldLine.bottom = rcFoldLine.top + 30;
						rcFoldLine.left = 100;
						rcFoldLine.right = 120;
						surface->FillRectangle(rcFoldLine, vs.styles[STYLE_CALLTIP].back);
						rcFoldLine.top -= 11;
						rcFoldLine.bottom = rcFoldLine.top + 32;
						rcFoldLine.left = 99;						
						rcFoldLine.right = 121;
						surface->FillRectangle(rcFoldLine, vs.styles[STYLE_CALLTIP].fore);
						rcFoldLine.top -= 10;
						rcFoldLine.bottom = rcFoldLine.top + 30;
						rcFoldLine.left = 100;						
						rcFoldLine.right = 120;
					...
					if ((expanded && (foldFlags & SC_FOLDFLAG_LINEAFTER_EXPANDED))
						...
						int pos = pdoc->LineStart(lineDoc); // LineEnd(lineDoc);
						
						

						int pos2 = pdoc->LineEnd(lineDoc);

						for (int i = pos; i < pos2; i++){
							char ch = pdoc->CharAt(i);
							if (ch != ' ' && ch != '\t'){
								pos = i;
								break;
							}
						...
						int posInLine = LocationFromPosition(pos).x;
						int posLineEnd = LocationFromPosition(pos2).x;
						
						//int hold = pdoc->LineFromPosition(pos);
						
						rcFoldLine = rcLine;
						rcFoldLine.left = posInLine-1;
						rcFoldLine.bottom - 1;
						//rcFoldLine.left = posInLine;// +2;	// Add an extra pixel so it doesn't sit to tightly on
														// the last character.
						rcFoldLine.right = posLineEnd - 1; // rcFoldLine.left + 29;

						// Throw in a pixel of padding to try to line it up more effectively.
						rcFoldLine.top += 1;
						

						//surface->FillRectangle(rcFoldLine, vs.styles[STYLE_CALLTIP].fore);
						//surface->RectangleDraw(rcFoldLine, NULL,);
						XYPOSITION posY =  ((rcFoldLine.right - rcFoldLine.left) / 2) + 1;
						rcFoldLine.right += 1;
						surface->AlphaRectangle(rcFoldLine, 0, vs.styles[STYLE_CALLTIP].back, 0, vs.styles[STYLE_CALLTIP].fore, 100, 0);
						//rcFoldLine.left = rcFoldLine.right - 28;
						rcFoldLine.bottom += 1;
						rcFoldLine.top -= 1;
						//surface->FillRectangle(rcFoldLine, vs.styles[STYLE_CALLTIP].back);
						// Finishg it off with ...
						...
						//rcFoldLine.top -= 10;
						//surface->DrawTextTransparent(rcFoldLine, vs.styles[STYLE_CALLTIP].font, posY, "...", 3, vs.styles[STYLE_CALLTIP].fore);
					}
				}
...
void Editor::NotifyDwelling(Point pt, bool state) {
	...
	// Check if the mouse is over a folded line...
...
void Editor::ButtonDownWithModifiers(Point pt, unsigned int curTime, int modifiers) {
	...
	if (((curTime - lastClickTime) < Platform::DoubleClickTime()) && Close(pt, lastClick)) {
		...
		if (selectionType == selWord) {
			int braceMatched = pdoc->BraceMatch(newPos.Position(), 0);
			int NextbraceMatched = -1;
			if(braceMatched == -1) 
				NextbraceMatched = pdoc->BraceMatch(newPos.Position()-1, 0);
			if(NextbraceMatched != -1) {
				char chBrace = pdoc->CharAt(newPos.Position()-1);
				int addCur = 1;
				if (chBrace == '(' || chBrace == '[' || chBrace == '{' || chBrace == '<') 
					addCur = 0;
				int c1 = newPos.Position() - 1 + (addCur ? 1 : 0);
				int c2 = NextbraceMatched + (addCur ? 0 : 1);
				SetSelection(c1,c2);
			} else if(braceMatched != -1) {
				char chBrace = pdoc->CharAt(newPos.Position());
				int addCur = 1;
				if (chBrace == '(' || chBrace == '[' || chBrace == '{' || chBrace == '<') 
					addCur = 0;
				int c1 = newPos.Position() + (addCur ? 1 : 0);
				int c2 = braceMatched + (addCur ? 0 : 1);
				SetSelection(c1,c2);
			} else {
