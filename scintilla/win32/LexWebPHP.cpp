...
/**
 * LexWebPHP is based directly on LexHTML. The primary change is that vba numbers have been
 * taken for the purpose of enhancing the PHP aspects of the lexer.
 */
...
// Based on the existing LexHTML lexer but taking the asp keys over for PHP
...
static const char *GetLastWord(Accessor &styler, unsigned int start, char *s, size_t sLen) {

	unsigned int i = sLen-1;
	for (; i > 0; i++) {
		char ch = static_cast<char>(styler.SafeGetCharAt(start + i));
		if ((i == 0) && !IsAWordStart(ch))
			break;
...
static const char * const phpscriptWordListDesc[] = {
	...
	"", //Unused
	"PHP keywords",
	"PHP keywords 2",	// to create some variation if desired for describer keywords (public, private, protected, static)
	0,
};

LexerModule lmWebPHP(SCLEX_HTML, ColouriseHTMLDoc, "hypertext", 0, htmlWordListDesc, 8);
//LexerModule lmXML(SCLEX_XML, ColouriseXMLDoc, "xml", 0, htmlWordListDesc, 8);
LexerModule lmPHPSCRIPT2(SCLEX_PHPSCRIPT, ColourisePHPScriptDoc, "phpscript", 0, phpscriptWordListDesc, 8);
